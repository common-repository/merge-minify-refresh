		...
		if(count($files) > 0)
		{
			foreach($files as $file)
			{
				...
				if(file_exists($script_path . '.accessed'))
				{
					...
					{
						$accessed = date(get_option('date_format') ?? "F j, Y", $accessed);
					}
					else
					{
						$accessed = 'Unknown';
					...
					*/
					...
					if(!file_exists($full_path) && !$min_exists)
					{
						...
						foreach( $handles[$i]['handles'] as $index => $handle)
						{
							...
							/*
								...
								We attempt conversion if the file is not considered to be UTF-8,
								but this may not work correctly so please make sure UTF-8 files
								are used.
							*/
							if(extension_loaded('mbstring') && !mb_check_encoding($scriptContent, "UTF-8"))
							{
								$encoding = mb_detect_encoding($scriptContent, [
									"UTF-8",
									"ASCII",
									'ISO-8859-1',
									"UTF-16",
								...
								if($encoding != "UTF-8")
								{
									$scriptContent = (WP_DEBUG ? "/* " . $encoding . " -> UTF-8 */ " : "") . mb_convert_encoding($scriptContent, "UTF-8", $encoding);
								}
							}
					...
					if($ext == 'js')
					{
						...
						if($min_exists)
						{
							...
							wp_register_script($newHandle, MMR_JS_CACHE_URL . $min_path, $handles[$i]["deps"], null, $args);
						}
						else
						{
							$this->http2push_reseource(MMR_JS_CACHE_URL . $file_path, 'script');
							wp_register_script($newHandle, MMR_JS_CACHE_URL . $file_path, $handles[$i]["deps"], null, $args);
						}
					}
					else
					{
						if($min_exists)
						{
							...
							wp_register_style($newHandle, MMR_CSS_CACHE_URL . $min_path, $handles[$i]["deps"], null, $handles[$i]['media']);
						}
						else
						{
							$this->http2push_reseource(MMR_CSS_CACHE_URL . $file_path, 'style');
							wp_register_style($newHandle, MMR_CSS_CACHE_URL . $file_path, $handles[$i]["deps"], null, $handles[$i]['media']);
						}
					}
